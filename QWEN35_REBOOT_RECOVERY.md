...
- first warm 32-token request: `6.2062 s`
- repeated warm 32-token request: `4.2446 s`
- repeated requests returned identical 32-token sequences
- steady server decode intervals: `13.77` to `13.96` token/s
- baseline-adjusted decode rate: about `14.5` token/s
- prior correct but untraced decode rate: about `1.06` token/s

The decode-only trace is therefore about 13.7 times faster than the untraced
control. The first real-weight request took `223.9375 s`, including ordinary
...
- Branch: `agent/qwen35-9b-upstream-kernels`
- Base: `origin/main` at `cf2f217`
