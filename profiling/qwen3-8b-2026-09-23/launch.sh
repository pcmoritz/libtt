...
exec env -u TT_METAL_RUNTIME_ROOT \
  ...
  JAX_PLATFORMS=tt \
  JAX_USE_SHARDY_PARTITIONER="${PROFILE_USE_SHARDY:-false}" \
  JAX_COMPILATION_CACHE_DIR="/tmp/libtt-profile/jax-cache-tp$TP" \
  TT_METAL_OPERATION_TIMEOUT_SECONDS=120 \
  /tmp/libtt-profile/venv/bin/python -m sgl_jax.launch_server \
