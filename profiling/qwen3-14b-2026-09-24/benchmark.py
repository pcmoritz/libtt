...
info_response = requests.get(base + '/get_server_info', timeout=10)
info_response.raise_for_status()
server_info = info_response.json()
if server_info.get('tp_size') != a.tp:
    raise RuntimeError(f"Expected TP{a.tp}, server reports TP{server_info.get('tp_size')}")
(out/'server-info.json').write_text(json.dumps(server_info, indent=2))
rows = []
prompts = {
