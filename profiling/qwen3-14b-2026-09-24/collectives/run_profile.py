...
root=Path('/tmp/libtt-profile/14b-ccl/profile')
...
for tp in ([int(x) for x in sys.argv[1:]] or [1,2]):
    ...
    with (root/f'server-tp{tp}.log').open('w') as log:
        server=subprocess.Popen(['bash','/tmp/libtt-profile/qwen3-14b/launch.sh',str(tp)],stdout=log,stderr=subprocess.STDOUT,env=dict(os.environ,PROFILE_VARIANT='ccl-direct-profile',LIBTT_TP2_MINIMAL_ALL_REDUCE='2',LIBTT_PROFILE_CSV=str(root/f'tp{tp}'/'programs')))
        try:
