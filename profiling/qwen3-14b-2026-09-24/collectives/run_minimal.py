...
root=Path('/tmp/libtt-profile/14b-ccl/minimal')
...
for tp in ([int(x) for x in sys.argv[1:]] or [1,2]):
    ...
    with (root/f'server-tp{tp}.log').open('w') as log:
        server=subprocess.Popen(['bash','/tmp/libtt-profile/qwen3-14b/launch.sh',str(tp)],stdout=log,stderr=subprocess.STDOUT,env=dict(os.environ,PROFILE_VARIANT='ccl-minimal',LIBTT_TP2_MINIMAL_ALL_REDUCE='1'))
        try:
            ...
            with (root/f'benchmark-tp{tp}.log').open('w') as bench:
                subprocess.run([python,'/tmp/libtt-profile/quick_bench.py','--tp',str(tp),'--output',str(root/f'tp{tp}')],stdout=bench,stderr=subprocess.STDOUT,check=True)
        finally:
