...
root=Path('/tmp/libtt-profile/14b-opt/recheck-final')
...
for tp in ([int(x) for x in sys.argv[1:]] or [1,2]):
    ...
    with (root/f'server-tp{tp}.log').open('w') as log:
        server=subprocess.Popen(['bash','/tmp/libtt-profile/qwen3-14b/launch.sh',str(tp)],stdout=log,stderr=subprocess.STDOUT,env=dict(os.environ,PROFILE_VARIANT='recheck-final'))
        try:
