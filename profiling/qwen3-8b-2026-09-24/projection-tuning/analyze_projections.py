"""Map warmed Qwen3-8B device programs to projections by verified trace order.

One decode iteration is 36 * [QKV, output, SwiGLU, down] + LM head.
The distinct SwiGLU kernel anchors the sequence. Incomplete or unexpected
iterations are excluded rather than assigned by duration or runtime id.
"""
import argparse
...
def analyze(directory, window_ms):
    ...
    for chip, unique in chips.items():
        ...
        swiglu = [i for i, r in enumerate(rows) if 'matmul_swiglu.cpp' in r['kernels']]
        heads = [a + 2 for a, b in zip(swiglu, swiglu[1:]) if b - a == 5]
        cycles = []
        boundaries = []
        for a, b in zip(heads, heads[1:]):
            cycle = rows[a + 1:b + 1]
            if len(cycle) != 145 or int(cycle[0]['start']) < cutoff:
                continue
            positions = [i for i, r in enumerate(cycle) if 'matmul_swiglu.cpp' in r['kernels']]
            if positions != list(range(2, 144, 4)):
                continue
            cycles.append(cycle)
        ...
        for cycle in cycles:
            ...
            for i, row in enumerate(cycle):
                label = ('qkv', 'attention_output', 'swiglu', 'down')[i % 4] if i < 144 else 'lm_head'
