...
Tile::Tile(std::array<uint32_t, 2> tile_shape, bool transpose_tile) : tile_shape(tile_shape) {
  ...
  const auto* choice = std::find_if(
      kTileFaceShapes.begin(), kTileFaceShapes.end(), [this](const auto& pair) {
...
uint32_t Tile::get_tile_size(const DataFormat& format) const {
  ...
  switch (format) {
    ...
    case DataFormat::Bfp2_b:
      return (tile_hw / 4) + aligned_exp_size;
    case DataFormat::Bfp4:
    case DataFormat::Bfp4_b:
      return (tile_hw / 2) + aligned_exp_size;
    case DataFormat::Bfp8:
    case DataFormat::Bfp8_b:
      return tile_hw + aligned_exp_size;
    case DataFormat::MxFp4:
      return tt::round_up(tile_hw / 32, kHostOnlyL1Alignment) + (tile_hw / 2);
    case DataFormat::MxFp6P:
    case DataFormat::MxFp6R:
    case DataFormat::MxFp8R:
    case DataFormat::MxFp8P:
      return tt::round_up(tile_hw / 32, kHostOnlyL1Alignment) + tile_hw;
    ...
    case DataFormat::Tf32:
      throw std::invalid_argument("TF32 unsupported");
...
std::optional<HostBuffer> DistributedHostBuffer::get_shard(
...
void DistributedHostBuffer::emplace_shard(
    ...
    const std::function<HostBuffer()>& produce_buffer) {
...
bool DistributedHostBuffer::is_local(const distributed::MeshCoordinate& coord) const {
  return shards_.is_local(coord);
}

DistributedHostBuffer DistributedHostBuffer::transform(
    const TransformFn& fn,
    ProcessShardExecutionPolicy /*policy*/) const {
  const auto& shards = shards_.values();
  ...
  for (size_t index : get_populated_shard_indices()) {
    transformed_shards[index] = distributed::MaybeRemote<Shard>::local(
        Shard{.buffer = fn(shards[index]->buffer), .is_populated = true});
  }
...
void DistributedHostBuffer::apply(
    const ApplyFn& fn,
    ProcessShardExecutionPolicy /*policy*/) const {
  const auto& shards = shards_.values();
  for (size_t index : get_populated_shard_indices()) {
...
const std::set<distributed::MeshCoordinate>& DistributedHostBuffer::shard_coords() const {
  return shard_coords_;
}

const std::shared_ptr<distributed::multihost::DistributedContext>& DistributedHostBuffer::context()
    const {
  return context_;
}
...
std::ostream& operator<<(std::ostream& os, const ShardSpec& spec) {
  ...
  switch (spec.orientation) {
    case ShardOrientation::ROW_MAJOR:
      os << "ROW_MAJOR";
      break;
    case ShardOrientation::COL_MAJOR:
      os << "COL_MAJOR";
      break;
  }
  os << "}";
...
tt::tt_metal::ShardSpec from_json_t<tt::tt_metal::ShardSpec>::operator()(const nlohmann::json&) const {
  TT_THROW("ShardSpec JSON parsing is not available in the host-only tt-metal overlay");
}
...
std::shared_ptr<PinnedMemory> PinnedMemoryCache::try_pin(
    ...
    const distributed::MeshCoordinateRangeSet&,
    HostBuffer&,
    bool) {
  return nullptr;
}
