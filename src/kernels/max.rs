...
impl Kernel<MaxProgramKey> for MaxBf16Kernel {
    fn program_key(&self) -> MaxProgramKey {
        self.key
    }
...
pub(crate) fn eltwise_max_bf16(
    ...
    if lhs.dtype != DType::Float16B || rhs.dtype != DType::Float16B {
        return Err(invalid_input(format!(
            "eltwise_max_bf16 requires bf16 inputs, got {:?} and {:?}",
            lhs.dtype, rhs.dtype
        )));
    }
...
fn bf16_program(key: MaxProgramKey) -> io::Result<Program> {
    ...
    Ok(Program {
        ...
        name: "eltwise_max_bf16".to_owned(),
