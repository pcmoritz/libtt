...
pub(crate) fn top_k(
    ...
    let name = name.into();
    // This is the sampling hot path: scan the logits in parallel, then reduce
    // the per-core winners with the existing top-1 finalizer.
...
fn top1_partial_program(key: Top1PartialProgramKey) -> io::Result<Program> {
    ...
    for (core_index, core) in key.cores.iter().enumerate() {
        let (tile_start, tile_count) = split_tile_range(key.input_tiles, core_index, n_cores)?;
        runtime_args.add_core(
            *core,
            vec![
                0,
    ...
    let runtime_args = runtime_args.build()?;
    Ok(Program {
        writer_kernel: top1_partial_writer_source(),
        compile: CompileConfig {
            cbs: vec![
...
fn top1_partial_writer_source() -> String {
    format!("#define TOPK_DTYPE_BFLOAT16\n#define TOPK_PARTIAL\n{WRITER}")
}
