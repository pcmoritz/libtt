...
pub(crate) fn matmul_dot_general(
    ...
    matmul_dot_general_with_grouped_head_views(
        device,
        lhs,
        rhs,
        lhs_logical_shape,
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn matmul_dot_general_with_grouped_head_views(
    device: &mut Device,
    lhs: &DramBuffer,
    rhs: &DramBuffer,
    lhs_shape: &[usize],
    ...
    validate_grouped_head_view(lhs_shape, lhs_grouped_head_view, "lhs")?;
    validate_grouped_head_view(rhs_shape, rhs_grouped_head_view, "rhs")?;
    ...
    let shape = dot_general_shape_with_grouped_head_views(
        lhs_shape,
        lhs_logical_shape,
    ...
    validate_tile_count(lhs, tiled_shape_tile_count(lhs_shape)?, "lhs")?;
    validate_tile_count(rhs, tiled_shape_tile_count(rhs_shape)?, "rhs")?;
...
#[allow(clippy::too_many_arguments)]
fn dot_general_shape_with_grouped_head_views(
    ...
    if lhs_grouped_head_view.is_some() || rhs_grouped_head_view.is_some() {
        return Err(invalid_input(
            "matmul grouped-head view is only supported for packed GQA attention",
        ));
    }
...
fn packed_gqa_shape(
    ...
    if group_size <= 1
        || grouped_dim != lhs_dims.batch[0]
        || lhs_dims.free[0] >= grouped_dim
        || grouped.logical_shape.as_slice() != lhs_shape
    {
        return Ok(None);
    }
    let kv_heads = lhs_physical_shape[grouped_dim];
    ...
    let m = lhs_physical_shape[lhs_dims.free[0]];
    ...
    Ok(Some(DotGeneralMatmulShape {
        ...
        rhs_view: packed_head_rhs_view(
            rhs_physical_shape,
            &rhs_internal_shape,
            k,
            group_size,
        )?,
        output_view: packed_head_output_view(
            output_shape,
            &packed_output_shape,
            m,
            group_size,
...
fn packed_head_rhs_view(
    ...
    if physical_shape.len() != 3 {
        return Err(invalid_input(
            "packed GQA matmul currently requires a rank-3 RHS",
        ));
    }
    let allocation_shape = tiled_allocation_shape(physical_shape)?;
    Ok(MatmulOperandView {
        kind: MatmulViewKind::PackedHead,
        rank: u32_value(view_shape.len(), "packed GQA RHS rank")?,
        batch_rank: 1,
        row_rank: 1,
        col_rank: 2,
        ...
        shape: padded_u32_array(view_shape, "packed GQA RHS shape")?,
...
fn packed_head_output_view(
    ...
    Ok(MatmulOperandView {
        kind: MatmulViewKind::PackedHead,
        rank: u32_value(view_shape.len(), "packed GQA output rank")?,
        batch_rank: 1,
        row_rank: 1,
        col_rank: 2,
        ...
        shape: padded_u32_array(view_shape, "packed GQA output shape")?,
