use crate::dram::{tiled_allocation_shape, TILE_C, TILE_R};
use std::io;

#[derive(Clone, Debug, Hash, PartialEq, Eq)]
pub(crate) struct ReshapeSourceView {
    pub(crate) rows: u32,
    pub(crate) cols: u32,
    pub(crate) tile_rows: u32,
    pub(crate) tiles_per_row: u32,
}

pub(crate) fn reshape_source_view(
    source_shape: &[usize],
    logical_shape: &[usize],
    name: &str,
) -> io::Result<ReshapeSourceView> {
    validate_same_volume(source_shape, logical_shape, name)?;

    let allocation_shape = tiled_allocation_shape(source_shape)?;
    let allocation_rank = allocation_shape.len();
    let (rows, cols) = matrix_dims(source_shape);
    Ok(ReshapeSourceView {
        rows: u32_arg(rows, &format!("{name} source rows"))?,
        cols: u32_arg(cols, &format!("{name} source cols"))?,
        tile_rows: u32_arg(
            allocation_shape[allocation_rank - 2] / TILE_R,
            &format!("{name} source tile rows"),
        )?,
        tiles_per_row: u32_arg(
            allocation_shape[allocation_rank - 1] / TILE_C,
            &format!("{name} source tiles per row"),
        )?,
    })
}

fn matrix_dims(shape: &[usize]) -> (usize, usize) {
    match shape.len() {
        0 => (1, 1),
        1 => (1, shape[0]),
        rank => (shape[rank - 2], shape[rank - 1]),
    }
}

fn validate_same_volume(lhs: &[usize], rhs: &[usize], name: &str) -> io::Result<()> {
    let lhs_volume = element_count(lhs).ok_or_else(|| invalid_input(format!("{name} overflow")))?;
    let rhs_volume = element_count(rhs).ok_or_else(|| invalid_input(format!("{name} overflow")))?;
    if lhs_volume != rhs_volume {
        return Err(invalid_input(format!(
            "{name} volume mismatch: source {lhs:?}, logical {rhs:?}"
        )));
    }
    Ok(())
}

fn element_count(shape: &[usize]) -> Option<usize> {
    shape
        .iter()
        .try_fold(1usize, |acc, &dim| acc.checked_mul(dim))
}

fn u32_arg(value: usize, name: &str) -> io::Result<u32> {
    u32::try_from(value)
        .map_err(|_| invalid_input(format!("{name} does not fit in u32: 0x{value:x}")))
}

fn invalid_input(message: impl Into<String>) -> io::Error {
    io::Error::new(io::ErrorKind::InvalidInput, message.into())
}
