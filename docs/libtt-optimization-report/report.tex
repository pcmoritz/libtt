...
{[}18{]}, TTNN {[}21{]}, TT-Metalium {[}16{]}, and device kernels, where
...
model {[}26{]}. The following sections describe the system boundary, the
...
core), and up to 32 GB of GDDR6 memory {[}8{]}. The host reaches the
...
SRAM and can overlap memory traffic with arithmetic {[}16, 17{]}. Matrix
...
transport used by higher runtimes {[}24{]}.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
TT-Metalium\strut
\end{minipage} & \begin{minipage}[t]{0.47\columnwidth}\raggedright
Allocates device resources, builds command programs, and compiles and
dispatches reader, compute, and writer kernels {[}16, 17{]}.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.47\columnwidth}\raggedright
TTNN\strut
\end{minipage} & \begin{minipage}[t]{0.47\columnwidth}\raggedright
...
{[}11{]}. The backend supplies platform configuration, model
...
\item
  ...
  loader, worker, model runner, or engine {[}11{]}.
\end{enumerate}
...
selected outputs to the host {[}23{]}. The vLLM generator similarly
...
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
Upstream SGLang-JAX remains above PJRT\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.30\columnwidth}\raggedright
Model\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
...
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
The upstream JAX Qwen model is traced to StableHLO\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.30\columnwidth}\raggedright
...
remain close to the upstream JAX ecosystem without giving up decode
performance.
...
{[}24{]}, SFPI {[}12{]}, TT-Metal, LLVM, StableHLO, TT-XLA {[}25{]},
...
framework frontends through TT-MLIR dialects to TT-Metalium {[}14,
19{]}. libtt builds a pinned part of that path as one artifact.
...
kernels, and host/device control {[}19{]}. Each optimization should use
...
concrete runtime API is chosen {[}19{]}.\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
...
mapping to TT-Metal kernels {[}19{]}.\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
...
{[}13{]}. Final dimensions are padded, so batch-one decode still
...
shards, memory space, and padding {[}20{]}.

We use BF16 activations and outputs with BFLOAT8\_B weights. BFLOAT8\_B
is block floating point: 16 values share an exponent {[}22{]}. It
...
Qwen3 is a decoder-only Transformer family {[}26{]}. A simplified layer
is:
...
RMSNorm and SwiGLU are standard model concepts {[}6, 27{]}. Prefill
...
around vLLM and TT-Transformers {[}15{]}. It enters the stack above TTNN
