...
mlir::LogicalResult runCleanupRewritePatterns(
    ...
    patterns.add<libtt::mlir_frontend::GQAMatmulFusing>(&context);
...
void fillMatmulGroupedHeadView(
    const libtt::mlir_frontend::GroupedHeadExpansion& expansion,
    tt::MatmulGroupedHeadView* grouped_head_view) {
    for (int64_t dim : expansion.logicalShape) {
        grouped_head_view->add_logical_shape(dim);
    }
    grouped_head_view->set_grouped_dimension(expansion.groupedDimension);
    grouped_head_view->set_group_size(expansion.groupSize);
}
...
bool addMatmulOperand(
    ...
    tt::MatmulGroupedHeadView* grouped_head_view) {
    if (auto expansion =
            libtt::mlir_frontend::matchPackedGqaMatmulLhsMarker(dot_op, value)) {
...
bool lowerToExecutable(FuncOp func, tt::Executable& executable, std::string& error) {
    ...
    for (mlir::Operation& op : func.front()) {
        ...
        if (auto custom_call_op = mlir::dyn_cast<mlir::stablehlo::CustomCallOp>(op)) {
            if (libtt::mlir_frontend::isGroupedHeadExpansionMarker(custom_call_op)) {
                continue;
            }
