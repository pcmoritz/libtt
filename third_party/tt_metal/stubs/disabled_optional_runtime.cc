...
std::vector<DPrintBufferInfo> DPrintServer::get_core_buffers(ChipId, const umd::CoreDescriptor&) const {
  return {};
}
...
FabricNodeId ControlPlane::get_fabric_node_id_from_physical_chip_id(ChipId physical_chip_id) const {
  return FabricNodeId(MeshId{0}, static_cast<uint32_t>(physical_chip_id));
}
...
std::vector<uint32_t> get_forwarding_link_indices(const FabricNodeId&, const FabricNodeId&) { return {}; }
...
uint32_t FabricBuilderContext::get_num_fabric_initialized_routers(ChipId) const { return 0; }
...
std::optional<std::pair<uint32_t, EDMStatus>> FabricBuilderContext::get_fabric_router_ready_address_and_signal()
    const {
  return std::nullopt;
}
...
FabricTensixCoreType FabricTensixDatamoverConfig::get_core_id_for_channel(ChipId, uint32_t) const {
  return FabricTensixCoreType::MUX;
}
CoreCoord FabricTensixDatamoverConfig::get_core_for_channel(ChipId, uint32_t) const { return CoreCoord{0, 0}; }
std::pair<uint32_t, uint32_t> FabricTensixDatamoverConfig::get_termination_address_and_signal(
    FabricTensixCoreType) const {
  return {0, 0};
}

FabricMuxConfig::FabricMuxConfig(uint8_t, uint8_t, uint8_t, uint8_t, size_t, size_t base_l1_address, CoreType) :
    memory_map_end_address_(base_l1_address) {}
std::vector<uint32_t> FabricMuxConfig::get_fabric_mux_compile_time_args_for_relay_mux() const { return {}; }
size_t FabricMuxConfig::get_memory_map_end_address() const { return memory_map_end_address_; }
uint8_t FabricMuxConfig::get_num_buffers(FabricMuxChannelType) const { return 0; }
size_t FabricMuxConfig::get_buffer_size_bytes(FabricMuxChannelType) const { return 0; }
size_t FabricMuxConfig::get_channel_base_address(FabricMuxChannelType, uint8_t) const { return 0; }
size_t FabricMuxConfig::get_connection_info_address(FabricMuxChannelType, uint8_t) const { return 0; }
size_t FabricMuxConfig::get_connection_handshake_address(FabricMuxChannelType, uint8_t) const { return 0; }
size_t FabricMuxConfig::get_flow_control_address(FabricMuxChannelType, uint8_t) const { return 0; }
size_t FabricMuxConfig::get_buffer_index_address(FabricMuxChannelType, uint8_t) const { return 0; }
...
size_t FabricMuxConfig::get_channel_credits_stream_id(FabricMuxChannelType, uint8_t) const { return 0; }

template <>
std::vector<uint32_t> FabricMuxConfig::get_fabric_mux_run_time_args<tt_metal::Program>(
    const FabricNodeId&, const FabricNodeId&, uint32_t, tt_metal::Program&, const CoreCoord&) const {
  return {};
}
