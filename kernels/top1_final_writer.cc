...
void kernel_main() {
  ...
  for (uint32_t tile_id = 0; tile_id < partial_count; ++tile_id) {
    ...
    uint32_t value_bits = pair_ptr[0];
    uint32_t index = pair_ptr[1];
    cb_push_back(cb_partial_pairs, 1);
    cb_pop_front(cb_partial_pairs, 1);

    uint16_t key = value_key(static_cast<uint16_t>(value_bits));
  ...
  store_value(values_l1_addr, 0, best_value);
  ...
  indices_ptr[0] = static_cast<int32_t>(best_index);
