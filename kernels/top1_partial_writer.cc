...
uint16_t value_key(uint16_t bits) {
  return static_cast<uint16_t>((bits & 0x8000u) != 0 ? ~bits : (bits ^ 0x8000u));
}
...
void kernel_main() {
  ...
  uint16_t best_key = 0;
  uint16_t best_value = BF16_NEG_INF;
  ...
  pair_ptr[0] = have_best ? best_value : BF16_NEG_INF;
  pair_ptr[1] = have_best ? best_index : 0xffffffffu;
