...
constexpr uint32_t MAX_RANK = REDUCE_GENERAL_MAX_RANK;

constexpr uint32_t ARG_INPUT_ADDR = 0;
constexpr uint32_t ARG_OUTPUT_TILE_OFFSET = 1;
constexpr uint32_t ARG_OUTPUT_TILE_COUNT = 2;
constexpr uint32_t ARG_OUTPUT_RANK = 3;
constexpr uint32_t ARG_INPUT_TILE_ROWS = 4;
constexpr uint32_t ARG_INPUT_TILES_PER_ROW = 5;
constexpr uint32_t ARG_REDUCE_DIM_SIZE = 6;
constexpr uint32_t ARG_OUTPUT_ROWS = 7;
constexpr uint32_t ARG_OUTPUT_COLS = 8;
constexpr uint32_t ARG_OUTPUT_TILES_PER_ROW = 9;
constexpr uint32_t ARG_OUTPUT_MATRIX_TILES = 10;
constexpr uint32_t ARG_OUTPUT_SHAPE = 11;
...
float read_input_value(uint32_t row, uint32_t col) {
  volatile tt_l1_ptr uint32_t *ptr =
      reinterpret_cast<volatile tt_l1_ptr uint32_t *>(get_read_ptr(tt::CBIndex::c_0));
  return bits_to_float(ptr[tile_element_index(row, col)]);
}
...
void write_output_value(float value, uint32_t row, uint32_t col) {
  ...
  ptr[tile_element_index(row, col)] = float_to_bits(value);
}
...
void kernel_main() {
  ...
  uint32_t input_rank = output_rank + 1;
    ...
    for (uint32_t row = 0; row < TILE_R; ++row) {
      ...
      for (uint32_t col = 0; col < TILE_C; ++col) {
        ...
        for (uint32_t dim = 0; dim < output_rank; ++dim) {
          input_indices[dim] = output_indices[dim];
        }

#if REDUCE_GENERAL_IS_SUM
        float acc = 0.0f;
#else
        float acc = 0.0f;
        bool have_value = false;
#endif
        for (uint32_t reduce_index = 0; reduce_index < A(ARG_REDUCE_DIM_SIZE); ++reduce_index) {
          input_indices[input_rank - 1] = reduce_index;
          uint32_t input_row = 0;
          uint32_t input_col = 0;
          uint32_t input_tile = tile_id_for_indices(
              input_indices, input_shape, input_rank, A(ARG_INPUT_TILE_ROWS),
              A(ARG_INPUT_TILES_PER_ROW), &input_row, &input_col);
          ensure_input_tile(input, input_tile, &loaded_input_tile);
          float value = read_input_value(input_row, input_col);
#if REDUCE_GENERAL_IS_SUM
          acc += value;
#else
          if (!have_value || value > acc) {
            acc = value;
            have_value = true;
          }
#endif
