...
constexpr DataFormat multiply_input_data_format(uint32_t cb_lhs, uint32_t cb_out) {
...
void MAIN {
  ...
  constexpr DataFormat input_format = multiply_input_data_format(cb_lhs, cb_out);

  unary_op_init_common(cb_lhs, cb_out);
  multiply_input_init<input_format>();
  ...
  for (uint32_t i = 0; i < n_tiles; ++i) {
    ...
    multiply_input_tile<input_format>(0, 1, 0);
    tile_regs_commit();
