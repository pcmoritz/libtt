...
constexpr uint32_t RANK = PAD_RANK;
constexpr uint32_t COORD_COUNT = RANK == 0 ? 1 : RANK;
constexpr uint32_t INPUT_SHAPE[COORD_COUNT] = PAD_INPUT_SHAPE;
constexpr uint32_t OUTPUT_SHAPE[COORD_COUNT] = PAD_OUTPUT_SHAPE;
constexpr uint32_t EDGE_PADDING_LOW[COORD_COUNT] = PAD_EDGE_PADDING_LOW;
constexpr uint32_t INTERIOR_PADDING[COORD_COUNT] = PAD_INTERIOR_PADDING;
constexpr uint32_t INPUT_TILE_ROWS = PAD_INPUT_TILE_ROWS;
constexpr uint32_t INPUT_TILES_PER_ROW = PAD_INPUT_TILES_PER_ROW;
constexpr uint32_t OUTPUT_TILE_ROWS = PAD_OUTPUT_TILE_ROWS;
constexpr uint32_t OUTPUT_TILES_PER_ROW = PAD_OUTPUT_TILES_PER_ROW;
constexpr bool DIRECT_COPY = PAD_DIRECT_COPY != 0;
using Element = PAD_ELEMENT_TYPE;
...
bool map_coord(uint32_t dim, uint32_t output, uint32_t *input) {
  if (output < EDGE_PADDING_LOW[dim]) {
    return false;
  }
  uint32_t shifted = output - EDGE_PADDING_LOW[dim];
  uint32_t step = INTERIOR_PADDING[dim] + 1;
  if (shifted % step != 0) {
    return false;
  }
  uint32_t candidate = shifted / step;
  if (candidate >= INPUT_SHAPE[dim]) {
    return false;
  }
  *input = candidate;
  return true;
...
Location input_location(const uint32_t base_output_coords[COORD_COUNT],
                        uint32_t output_row, uint32_t output_col) {
  if constexpr (RANK == 0) {
    return Location{true, 0, 0, 0};
  } else if constexpr (RANK == 1) {
    uint32_t input_col = 0;
    if (!map_coord(0, output_col, &input_col)) {
      return Location{false, 0, 0, 0};
    }
    return Location{true, input_col / TILE_C, 0, input_col % TILE_C};
  } else {
    uint32_t input_batch = 0;
    for (uint32_t dim = 0; dim < RANK - 2; ++dim) {
      uint32_t coord = 0;
      if (!map_coord(dim, output_coord(dim, base_output_coords, output_row, output_col), &coord)) {
        return Location{false, 0, 0, 0};
      }
    ...
    uint32_t input_col = 0;
    if (!map_coord(RANK - 2, output_row, &input_row) ||
        !map_coord(RANK - 1, output_col, &input_col)) {
      return Location{false, 0, 0, 0};
    }
    ...
    uint32_t input_tile =
        ...
        input_tile_col;
    return Location{true, input_tile, input_row % TILE_R, input_col % TILE_C};
  }
}
...
void kernel_main() {
  ...
  const InterleavedAddrGenFast<true> padding = {
      .bank_base_address = padding_value_addr,
      .page_size = get_tile_size(cb_padding),
      .data_format = get_dataformat(cb_padding),
  };

  cb_reserve_back(cb_padding, 1);
  noc_async_read_tile(0, padding, get_write_ptr(cb_padding));
  noc_async_read_barrier();
  cb_push_back(cb_padding, 1);
  cb_wait_front(cb_padding, 1);
  volatile tt_l1_ptr Element *padding_tile =
      reinterpret_cast<volatile tt_l1_ptr Element *>(get_read_ptr(cb_padding));
  Element padding_value = padding_tile[tile_element_index(0, 0)];
  ...
  for (uint32_t tile = 0; tile < output_tile_count; ++tile) {
    ...
    fill_tile(cb_output, padding_value);
