void kernel_main() {
  ...
  const uint32_t batch_count = A(9);
  const uint32_t head_tiles = (head_dim + TILE_C - 1) / TILE_C;
  const uint32_t kt = (key_tokens + TILE_C - 1) / TILE_C;
  ...
  for (uint32_t local_work = 0; local_work < output_tile_count; ++local_work) {
    ...
    for (uint32_t k_tile = 0; k_tile < kt; ++k_tile) {
      ...
      for (uint32_t col = 0; col < TILE_C; ++col) {
        ...
        const uint32_t lhs_tile = lhs_prefix * head_tiles + head_tile;
    ...
    for (uint32_t group = 0; group < groups; ++group) {
      for (uint32_t k_tile = 0; k_tile < kt; ++k_tile) {
        ...
        const uint32_t rhs_tile = (rhs_prefix * output_tiles_per_row + t_tile) * kt + k_tile;
        cb_reserve_back(cb_tmp, one_tile);
