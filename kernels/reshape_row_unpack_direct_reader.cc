...
void write_row_segment(const InterleavedAddrGenFast<true> &output, uint32_t output_tile,
                       uint32_t output_row, uint32_t input_row, uint32_t input_l1_addr,
...
void kernel_main() {
  ...
  for (uint32_t i = 0; i < output_tile_count; ++i) {
    uint32_t fragment = output_tile_offset + i;
    uint32_t global_output_row = fragment / output_tiles_per_row;
    uint32_t output_tile_col = fragment - global_output_row * output_tiles_per_row;
    uint32_t output_matrix = global_output_row / output_rows;
    uint32_t output_row = global_output_row - output_matrix * output_rows;
    uint32_t output_tile = output_matrix * output_matrix_tiles +
    ...
    uint32_t input_tile = global_output_row * input_matrix_tiles +
                          (input_row / TILE_R) * input_tiles_per_row + input_tile_col;
    ...
    write_row_segment(output, output_tile, output_row % TILE_R, input_row % TILE_R,
                      input_l1_addr, element_bytes);
