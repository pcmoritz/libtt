...
constexpr uint32_t RANK = REDUCE_WINDOW_RANK;
constexpr uint32_t INPUT_SHAPE[RANK] = REDUCE_WINDOW_INPUT_SHAPE;
constexpr uint32_t OUTPUT_SHAPE[RANK] = REDUCE_WINDOW_OUTPUT_SHAPE;
constexpr uint32_t WINDOW_DIMENSIONS[RANK] = REDUCE_WINDOW_WINDOW_DIMENSIONS;
constexpr uint32_t WINDOW_STRIDES[RANK] = REDUCE_WINDOW_WINDOW_STRIDES;
constexpr uint32_t BASE_DILATIONS[RANK] = REDUCE_WINDOW_BASE_DILATIONS;
constexpr uint32_t WINDOW_DILATIONS[RANK] = REDUCE_WINDOW_WINDOW_DILATIONS;
constexpr uint32_t PADDING_LOW[RANK] = REDUCE_WINDOW_PADDING_LOW;
constexpr uint32_t INPUT_TILE_ROWS = REDUCE_WINDOW_INPUT_TILE_ROWS;
constexpr uint32_t INPUT_TILES_PER_ROW = REDUCE_WINDOW_INPUT_TILES_PER_ROW;
constexpr uint32_t OUTPUT_TILE_ROWS = REDUCE_WINDOW_OUTPUT_TILE_ROWS;
constexpr uint32_t OUTPUT_TILES_PER_ROW = REDUCE_WINDOW_OUTPUT_TILES_PER_ROW;
constexpr uint32_t WINDOW_ELEMENTS = REDUCE_WINDOW_WINDOW_ELEMENTS;
using Element = REDUCE_WINDOW_ELEMENT_TYPE;
constexpr Element IDENTITY = static_cast<Element>(0);
...
void decode_output_batch(uint32_t batch, uint32_t coords[RANK]) {
...
uint32_t output_coord(uint32_t dim, const uint32_t base_coords[RANK],
...
void decode_window_coords(uint32_t linear, uint32_t coords[RANK]) {
  for (uint32_t index = 0; index < RANK; ++index) {
    uint32_t dim = RANK - 1 - index;
    coords[dim] = linear % WINDOW_DIMENSIONS[dim];
    linear /= WINDOW_DIMENSIONS[dim];
  }
}

bool map_input_coords(const uint32_t output_coords[RANK],
                      const uint32_t window_coords[RANK],
                      uint32_t input_coords[RANK]) {
  for (uint32_t dim = 0; dim < RANK; ++dim) {
    int64_t dilated_index =
        static_cast<int64_t>(output_coords[dim]) * WINDOW_STRIDES[dim] +
        static_cast<int64_t>(window_coords[dim]) * WINDOW_DILATIONS[dim] -
        static_cast<int64_t>(PADDING_LOW[dim]);
    if (dilated_index < 0) {
      return false;
    }
    if (dilated_index % BASE_DILATIONS[dim] != 0) {
      return false;
    }
    uint32_t input = static_cast<uint32_t>(dilated_index / BASE_DILATIONS[dim]);
    if (input >= INPUT_SHAPE[dim]) {
      return false;
    }
    input_coords[dim] = input;
  }
  return true;
}

Location input_location(const uint32_t coords[RANK]) {
  if constexpr (RANK == 1) {
...
void kernel_main() {
  ...
  for (uint32_t tile = 0; tile < output_tile_count; ++tile) {
    ...
    uint32_t base_output_coords[RANK];
    ...
    for (uint32_t row = 0; row < row_count; ++row) {
      ...
      for (uint32_t col = 0; col < col_count; ++col) {
        ...
        uint32_t output_coords[RANK];
        for (uint32_t dim = 0; dim < RANK; ++dim) {
          output_coords[dim] = output_coord(dim, base_output_coords, output_row, output_col);
        }

        Element value = IDENTITY;
        for (uint32_t window_index = 0; window_index < WINDOW_ELEMENTS; ++window_index) {
          uint32_t window_coords[RANK];
          uint32_t input_coords[RANK];
          decode_window_coords(window_index, window_coords);
          if (map_input_coords(output_coords, window_coords, input_coords)) {
