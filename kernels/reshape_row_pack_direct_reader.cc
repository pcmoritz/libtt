...
void kernel_main() {
  ...
  uint32_t input_tiles_per_row = get_arg_val<uint32_t>(4);
  uint32_t output_tiles_per_row = get_arg_val<uint32_t>(5);
  uint32_t output_rows = get_arg_val<uint32_t>(6);
  uint32_t output_tile_rows = get_arg_val<uint32_t>(7);
  ...
  const uint32_t fragments_per_batch = output_rows * output_tiles_per_row;
  ...
  for (uint32_t i = 0; i < fragment_count; ++i) {
    ...
    uint32_t batch = fragment / fragments_per_batch;
    uint32_t batch_fragment = fragment - batch * fragments_per_batch;
    uint32_t output_row = batch_fragment / output_tiles_per_row;
    uint32_t output_tile_col = batch_fragment - output_row * output_tiles_per_row;

    uint32_t input_tile = (batch / TILE_R) * input_tiles_per_row +
                          output_row * output_tiles_per_row + output_tile_col;
    uint32_t output_tile = batch * output_matrix_tiles +
                           (output_row / TILE_R) * output_tiles_per_row + output_tile_col;
    ...
    write_row_segment(output, output_tile, output_row % TILE_R, input_l1_addr, batch % TILE_R,
                      element_bytes);
