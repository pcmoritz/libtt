...
void kernel_main() {
  ...
  uint32_t lhs_single_tile = get_arg_val<uint32_t>(4);
  uint32_t rhs_single_tile = get_arg_val<uint32_t>(5);
  ...
  for (uint32_t i = 0; i < n_tiles; ++i) {
    ...
    noc_async_read_tile(lhs_single_tile ? 0 : offset + i, lhs, get_write_ptr(cb_lhs));
    noc_async_read_tile(rhs_single_tile ? 0 : offset + i, rhs, get_write_ptr(cb_rhs));
